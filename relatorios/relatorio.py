...
# Substitua o código de exportação por este:
...
def criar_link_download(pdf_bytes, filename):
    """Gera um link para download do PDF gerado"""
    b64 = base64.b64encode(pdf_bytes).decode()
    href = f'<a href="data:application/pdf;base64,{b64}" download="{filename}" style="text-decoration:none; color:white; background-color:#1E88E5; padding:10px 15px; border-radius:4px; font-weight:bold;">📥 Download Relatório PDF</a>'
    return href

# Função para converter figura plotly em imagem
def plotly_fig_to_image(fig, width=750, height=400):
    """Converte figura do plotly para imagem para inclusão no PDF"""
    try:
        img_bytes = pio.to_image(fig, format="png", width=width, height=height, scale=2)
        img = Image(BytesIO(img_bytes))
        img.drawHeight = height / 72 * inch  # Convert pixels to inches
        img.drawWidth = width / 72 * inch
        return img
    except Exception as e:
        st.error(f"Erro ao converter gráfico para imagem: {str(e)}")
        return None

# Adicionar botão de exportação na barra lateral
st.sidebar.title("Exportação Avançada")

if st.sidebar.button("📊 Gerar PDF Melhorado"):
    try:
        with st.spinner("Gerando relatório PDF avançado..."):
            # Nome do arquivo
            data_atual = datetime.now().strftime("%Y%m%d_%H%M%S")
            nome_arquivo = f"relatorio_producao_{data_atual}.pdf"
            ...
            # Adicionar estilo para título centralizado (esse nome não existe no stylesheet padrão)
            ...
            # Modificar os estilos existentes em vez de tentar criar novos
            ...
            # Adicionar novo estilo com nome único
            styles.add(ParagraphStyle(
                name='NormalText',
            ...
            elements.append(Paragraph(
                f'<font color="#1E88E5">Relatório de Produção</font>',
            ...
            # Adicionar conteúdo do relatório ao PDF
            if not resumo_turno.empty:
                # Adicionar título
                elements.append(Paragraph("📊 Resumo de Produção por Turno", styles['Heading2']))
                
                # Criar tabela com os dados
                data = [["Centro", "Turno", "Produzido", "Eficiência (%)"]]
                for _, row in resumo_turno.iterrows():
                    data.append([
                        row["Centro Trabalho"],
                        row["Turno"],
                        f"{int(row['Produzido']):,}".replace(",", "."),
                        f"{row['Eficiencia_%']:.2f}" if pd.notna(row["Eficiencia_%"]) else "—"
                    ])
                
                tabela_resumo = Table(data)
            ...
            st.sidebar.markdown(criar_link_download(pdf_bytes, nome_arquivo), unsafe_allow_html=True)
